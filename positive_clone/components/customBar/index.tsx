...
const StyledBar: React.FC<{ scrollTop?: number }> = ({ scrollTop }) => {
    ...
    useEffect(() => {
        ...
        referenceSecond.current?.addEventListener("click", (e: any) => {
            setState({ ...state, logout: true });
        });
        return () => {
            referenceElement.current?.removeEventListener("click", (e) => {
                setState({ ...state, cart: true });
            });
            referenceSecond.current?.removeEventListener("click", (e: any) => {
                setState({ ...state, logout: true });
            });
        }
    });
