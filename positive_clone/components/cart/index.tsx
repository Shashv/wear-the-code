...
const CustomDrawer: React.FC<IDrawer> = ({ open, width, height, list, closeDrawer, reduxAdd, reduxSubtract, reviewCart }) => {
    ...
    return (
        <>
                ...
                reviewCart ?
                    <Drawer open={open} anchor={"right"} className={styles.customdrawer} onClose={closeDrawer} transitionDuration={350} PaperProps={{
                        sx: {
                            ...
                            height,
                            padding: 2,
                            backgroundColor: theme.light ? "magenta" : "#fff"
                        }
                    }}>
                        <StyledModal open={clearCartConfirmation} title="Are you sure you want to clear cart ?" purpose={"Clear Items"}
                            confirmProcess={Object.keys(buyProduct).length > 0 ? confirmClearBuyProducts : confirmClearCart} content={<>Shashvat Gupta</>} closeModal={() => setClearCartConfirmation(false)} />
                            ...
                            </div>
                            {
                                Object.keys(buyProduct).length > 0 ?
                                    <List className="container-fluid">
                                        {Object.keys(buyProduct).map((buyProducy, index) => <ListItem sx={{ marginTop: "12.4px", marginBottom: "12.4px" }} disableGutters key={`${buyProduct}-${index}`}>
            ...
            {/* custom toast if you want tom use instead use react-toastify */}
