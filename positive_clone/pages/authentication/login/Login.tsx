...
const Login: NextPage = () => {
    ...
    const details = (data: FieldValues) => {
        fetch("/api/loginn", { method: 'POST', body: JSON.stringify(data) }).then(response => response.json()).then(response => {
    ...
    const registerPassword = register("password", {
        required: true, max: 25, onChange(event) {
            handlePasswordChange(event);
        },
    });
    const registerEmail = register("email", {
        required: true, max: 45, onChange(event) {
    ...
    return (
        <section className={theme.dark ? "bg-gray-50 dark:bg-gray-200" : "bg-pink-300"}>
            <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                <p className="flex items-center text-2xl font-semibold text-pink-600">
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(details)}>
                            <div className="form-group position-relative">
                                ...
                                {errors.email && <Typography className="absolute top-25 left-3" color={"red"} fontSize={"12px"}>Email is required*</Typography>}
                            </div>
                            <div className="form-group position-relative">
                                <label htmlFor="password" className="block mb-2 text-sm font-medium text-pink-500">Password</label>
                                <input type="password" id="password" placeholder="••••••••" {...registerPassword} className="border border-pink-300 text-pink-500 rounded-lg focus:ring-ppink-600 focus:border-pink-600 block w-full p-2.5" />
                                {errors.password && <Typography className="position-absolute left-3 top-21" fontSize={"12px"} color={"red"}>{"Password is required"}*</Typography>}
                            </div>
                            <div className="flex items-center justify-between">
                                ...
                                <a className="text-sm font-medium text-pink-600">Forgot password?</a>
                            </div>
